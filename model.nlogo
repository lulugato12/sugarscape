globals [
  gini-index-reserve
  lorenz-points
  maximum-spice-endowment ;; ! should i add a slider? or remove the sliders for sugar?
...
patches-own [
  ...
  pspice          ;; the amount of spice on this patch
  max-psugar      ;; the maximum amount of sugar that can be on this patch
  max-pspice      ;; the maximum amount of spice that can be on this patch
]
...
to setup-patches
  ...
  foreach sort patches [ p ->
    ask p [
      ...
      set max-pspice 9 - max-psugar ;; set the amount of spice to 9 - maxpsugar ;; there'd be places where spice is 0 always
...
to turtle-eat ;; turtle procedure
  ...
  set sugar (sugar - metabolism_su + psugar)
  set spice (spice - metabolism_sp + pspice)
  set psugar 0
  set pspice 0
end

to exchange ;; turtle procedure
  ;; find possible neighbors

  ;; for all neighbors and itself, compute mrs

  ;; if at least one has different mrs...
  ;; calculate geometric mean

  ;; spice flows from agent with higher mrs to lower, vice versa with sugar
  ;; if p > 1 -> p units of spice per 1 of sugar
  ;; if p < 1 -> 1/p units of sugar per 1 of spice

  ;; if welfare_0 < welfare_1, exchange
...
to-report mrs ;; turtle procedure
  report (metabolism_su * spice) / (metabolism_sp * sugar)
end
...
to color-agents-by-metabolism ;; turtle procedure
  set color red + (metabolism_su - 2.5)
end
